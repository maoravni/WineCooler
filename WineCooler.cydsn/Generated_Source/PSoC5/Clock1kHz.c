...
void Clock1kHz_Start(void) 
{
    ...
    Clock1kHz_CLKEN |= Clock1kHz_CLKEN_MASK;
	Clock1kHz_CLKSTBY |= Clock1kHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock1kHz_Stop
********************************************************************************
*
* Summary:
...
void Clock1kHz_Stop(void) 
{
    ...
    Clock1kHz_CLKEN &= (uint8)(~Clock1kHz_CLKEN_MASK);
	Clock1kHz_CLKSTBY &= (uint8)(~Clock1kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock1kHz_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock1kHz_StopBlock(void) 
{
    if ((Clock1kHz_CLKEN & Clock1kHz_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock1kHz__CFG3)
        CLK_DIST_AMASK = Clock1kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock1kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock1kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock1kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock1kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock1kHz_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock1kHz_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock1kHz_CLKSTBY &= (uint8)(~Clock1kHz_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock1kHz_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock1kHz_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock1kHz_GetSourceRegister();
    uint16 oldDivider = Clock1kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock1kHz_CLKEN & Clock1kHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock1kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock1kHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock1kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock1kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock1kHz_SetModeRegister(uint8 modeBitMask) 
{
    Clock1kHz_MOD_SRC |= modeBitMask & (uint8)Clock1kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock1kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock1kHz_ClearModeRegister(uint8 modeBitMask) 
{
    Clock1kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock1kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock1kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock1kHz_GetModeRegister(void) 
{
    return Clock1kHz_MOD_SRC & (uint8)(Clock1kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock1kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock1kHz_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock1kHz_GetDividerRegister();
    uint8 oldSrc = Clock1kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock1kHz_MOD_SRC |= CYCLK_SSS;
        Clock1kHz_MOD_SRC =
            (Clock1kHz_MOD_SRC & (uint8)(~Clock1kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock1kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock1kHz_GetSourceRegister(void) 
{
    return Clock1kHz_MOD_SRC & Clock1kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock1kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock1kHz_SetPhaseRegister(uint8 clkPhase) 
{
    Clock1kHz_PHASE = clkPhase & Clock1kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock1kHz_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock1kHz_GetPhaseRegister(void) 
{
    return Clock1kHz_PHASE & Clock1kHz_PHASE_MASK;
}
